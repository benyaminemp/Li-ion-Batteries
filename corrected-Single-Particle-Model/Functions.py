...
def D_c(c):
    """ Electrolyte diffusion coefficient function. """
    return (2.646e-10 * (0.2667 * c**3 - 1.2983 * c**2 + 1.7919 * c + 0.1726) / c) / Param.params["Dc"]
def eta_val(cs, ce):
    """ Overpotential for first cathode material. """
    value = NDParam.params["G"] / (2 * NDParam.params["Upsilon"] * np.sqrt((1 - cs) * cs * ce))
    return 2 * np.arcsinh(value)
def kappa(c):
    """ Electrolyte conductivity function. """
    return (0.2667 * c**3 - 1.2983 * c**2 + 1.7919 * c + 0.1726) / Param.params["kappa"]
